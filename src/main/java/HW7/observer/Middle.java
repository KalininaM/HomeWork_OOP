...
public class Middle extends JobSeeker implements Observer{
    ...
    public void receiveOffer(Vacancy vacancy) {
        if (this.salary <= vacancy.getSalary()){
            System.out.printf("Middle %s: Мне нужна эта работа! Желаемая зп %d (company: %s; speciality: " + speciality + "; salary: %d)\n", name,
                    salary, vacancy.getCompany().getName(), vacancy.getSalary());
            setRating();
            vacancy.getCompany().setJobSeekersList(this);
        }
        else {
            System.out.printf("Middle %s: Я найду работу получше! Желаемая зп %d (company: %s; speciality: " + speciality + "; salary: %d)\n", name,
                    salary, vacancy.getCompany().getName(), vacancy.getSalary());
        }
    }
